...
extract_files() {
    ...
    if [ -d "$source_dir" ]; then
        ...
        echo "Source directory '$source_dir' does not exist $ERROR"
...
# Extract files from 'theme' directory to ~/.themes and log output
extract_files "theme" ~/.themes 2>&1 | tee -a "$LOG"

# Extract files from 'icon' directory to ~/.icons and log output
extract_files "icon" ~/.icons 2>&1 | tee -a "$LOG"
